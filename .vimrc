...
" Make CTRL-W change buffers in insert mode.
imap <C-w> <NOP>
imap <C-w><C-w> <ESC><C-w><C-w>

" dbext *********************************************************************
let g:dbext_default_profile_mysqlconfig = 'type=MYSQL:user=root:passwd=:port=3307:dbname=net'
let g:dbext_default_profile_mysqldata = 'type=MYSQL:user=root:passwd=:port=3306:dbname=net'

" default to the mysqldata
let g:dbext_default_profile = 'mysqldata'

" Number of lines in the result window.
let g:dbext_default_buffer_lines = 40

" Visual mode comment *******************************************************
vmap c :s@^\t*@&//@<CR>:let @/ = ""<CR>
vmap C :s@\(^\t*\)//@\1@<CR>:let @/ = ""<CR>
...
highlight Folded ctermbg=DarkGrey ctermfg=LightBlue
highlight FoldColumn ctermbg=LightBlue ctermfg=Cyan

" Grep **********************************************************************
nmap <silent><C-G> :Grep <cword> *<CR>

" Fast colon ****************************************************************
nmap ; :

" ctags *********************************************************************
set tags=/usr/local/src/SevOneNMS/tags
...
"if has("cscope")
if 0
	" Load the cscope database
	cs add /usr/local/src/SevOneNMS/cscope.out
	" Use F11 to recreate cscope database.
	nmap <F11> :!find /usr/local/src/SevOneNMS/ -iname '*.c' -o -iname '*.cpp' -o -iname '*.h' -o -iname '*.hpp' > /usr/local/src/SevOneNMS/cscope.files<CR>
	   \:!cscope -b -i /usr/local/src/SevOneNMS/cscope.files -f /usr/local/src/SevOneNMS/cscope.out<CR>
	   \cs kill -1<CR>
	   \:cs add /usr/local/src/SevOneNMS/cscopes.out<CR>
...
" Bash type completion
set completeopt=longest,menuone
" Make pop-up more useful
...
"autocmd Filetype c imap { {<CR>}<Esc>O
"imap ( ()<Left>
"imap [ []<Left>
...
"toggle folding
nnoremap \tf :set nofoldenable!<Bar> set nofoldenable?<CR>
"nnoremap \tf :set fold!<Bar> set fold?<CR>
...
" mulfords maps cause he's a crip
map ff zc
map fo zo
